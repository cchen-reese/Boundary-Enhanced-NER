...
class WordRep(nn.Module):
    def __init__(self, data):
        ...
        if self.use_char:
            ...
            self.char_features_extractor = data.char_feature_extractor
            ...
            elif data.char_feature_extractor == "attention":
                self.char_feature = MultiHeadedAttention(
                    head_count = 3,
                    model_dim = self.char_embedding_dim,
                    dropout = 0.5
                    )
            else:
    ...
    def forward(self, word_inputs,feature_inputs, word_seq_lengths, char_inputs, char_seq_lengths, char_seq_recover):
        ...
        if self.use_char:
            if self.char_features_extractor == "attention":
                char_embeds = self.drop(self.char_embeddings(char_inputs))
                char_features,_ = self.char_feature(char_embeds, char_embeds, char_embeds)
                char_features = torch.mean(char_features, 1, True)
                char_features = char_features[char_seq_recover]
                char_features = char_features.view(batch_size, sent_len, self.char_hidden_dim)
            else:
