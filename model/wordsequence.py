...
class WordSequence(nn.Module):
    def __init__(self, data):
        ...
        # GCN
        self.use_gcn = data.use_gcn
        if self.use_gcn:
            self.gcn_type = data.HP_gcn_type
            ...
            if self.gcn_type == 'gcn':
                ...
                self.gat = GAT(data.HP_hidden_dim, data.HP_hidden_dim//3, data.label_alphabet_size, data.HP_dropout, \
                               alpha=0.1, nheads=3, layer=self.gcn_layer)
    ...
    def forward(self, word_inputs, feature_inputs, word_seq_lengths, char_inputs, char_seq_lengths, char_seq_recover, batch_graph):
        ...
        if self.word_feature_extractor == "CNN":
            ...
            x = self.word2star(word_represent)
            h = self.posi(x)
        ...
        if self.use_gcn == True:
            if self.gcn_type == 'gcn':
                ...
                gcn_out = self.gat(feature_out, batch_graph)
            net_out = gcn_out
        else:
            net_out = feature_out
        ...
        outputs = self.weight1 * net_out + self.weight2 * heads_feature + self.weight3 * tails_feature
