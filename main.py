...
def evaluate(data, model, name, nbest=None):
    ...
    for batch_id in range(total_batch):
        ...
        batch_label, batch_head, batch_tail, batch_graph, mask = batchify_with_label(instance, data.HP_gpu, False, data.sentence_classification)
        if nbest and not data.sentence_classification:
            scores, nbest_tag_seq = model.decode_nbest(batch_word, batch_features, batch_wordlen, batch_char, batch_charlen, batch_charrecover, batch_graph, mask, nbest)
            ...
            tag_seq = model(batch_word, batch_features, batch_wordlen, batch_char, batch_charlen, batch_charrecover, batch_graph, mask)
        # print("tag:",tag_seq)
...
def batchify_sequence_labeling_with_label(input_batch_list, gpu, if_train=True):
    """
        input: list of words, chars and labels, various length. [[words, features, chars, labels],[words, features, chars,labels],...]
            ...
            heads:      (batch_size, sent_len)
            tails:      (batch_size, sent_len)
            denp_heads:      (batch, sent_len)
            denp_deprel:     (batch, sent_len)
    ...
    heads = [sent[4] for sent in input_batch_list]
    tails = [sent[5] for sent in input_batch_list]
    denp_heads = [sent[6] for sent in input_batch_list]

    word_seq_lengths = torch.LongTensor(list(map(len, words)))
    max_seq_len = word_seq_lengths.max().item()
    word_seq_tensor = torch.zeros((batch_size, max_seq_len), requires_grad=if_train).long()
    label_seq_tensor = torch.zeros((batch_size, max_seq_len), requires_grad=if_train).long()
    heads_seq_tensor = torch.zeros((batch_size, max_seq_len), requires_grad=if_train).long()
    tails_seq_tensor = torch.zeros((batch_size, max_seq_len), requires_grad=if_train).long()
    dependency_matrix_tensor = torch.zeros((batch_size, max_seq_len, max_seq_len)).float()

    batch_dependency_matrix = []
    for d_head in denp_heads:
        seq_len = len(d_head)
        dependency_matrix = np.zeros((seq_len, seq_len), dtype=int)
        for idx, rela in enumerate(d_head):
            rela = int(rela)
            if rela > 0:
                dependency_matrix[idx][rela - 1] = 1
        frow_matrix = np.eye(seq_len, dtype=int)
        add_matrix1 = np.eye(seq_len, k=1, dtype=int)
        add_matrix2 = np.eye(seq_len, k=-1, dtype=int)
        dependency_matrix = dependency_matrix + frow_matrix + add_matrix1 + add_matrix2
        batch_dependency_matrix.append(dependency_matrix)
    ...
    for idx, (seq, label, head, tail, dependency_matrix, seqlen) in enumerate(zip(words, labels, heads, tails, batch_dependency_matrix, word_seq_lengths)):
        seqlen = seqlen.item()
        word_seq_tensor[idx, :seqlen] = torch.LongTensor(seq)
        label_seq_tensor[idx, :seqlen] = torch.LongTensor(label)
        heads_seq_tensor[idx, :seqlen] = torch.LongTensor(head)
        tails_seq_tensor[idx, :seqlen] = torch.LongTensor(tail)
        dependency_matrix_tensor[idx, :seqlen, :seqlen] = torch.LongTensor(dependency_matrix)
    ...
    heads_seq_tensor = heads_seq_tensor[word_perm_idx]
    tails_seq_tensor = tails_seq_tensor[word_perm_idx]
    dependency_matrix_tensor = dependency_matrix_tensor[word_perm_idx]
    ...
    if gpu:
        ...
        heads_seq_tensor = heads_seq_tensor.cuda()
        tails_seq_tensor = tails_seq_tensor.cuda()
        dependency_matrix_tensor = dependency_matrix_tensor.cuda()
        char_seq_tensor = char_seq_tensor.cuda()
        char_seq_recover = char_seq_recover.cuda()
        mask = mask.cuda()
    return word_seq_tensor, feature_seq_tensors, word_seq_lengths, word_seq_recover, \
           char_seq_tensor, char_seq_lengths, char_seq_recover, label_seq_tensor, \
           heads_seq_tensor, tails_seq_tensor, dependency_matrix_tensor, mask
...
def train(data):
    ...
    elif data.optimizer.lower() == "adabound":
        optimizer = adabound.AdaBound(model.parameters(), lr=data.HP_lr, weight_decay=data.HP_l2)
    else:
    ...
    for idx in range(data.HP_iteration):
        ...
        for batch_id in range(total_batch):
            ...
            batch_label, batch_head, batch_tail, batch_graph, mask  = \
                batchify_with_label(instance, data.HP_gpu, True, data.sentence_classification)
            instance_count += 1
            loss, tag_seq = model.calculate_loss(batch_word, batch_features, batch_wordlen, batch_char, batch_charlen, batch_charrecover, \
                                                 batch_label, batch_head, batch_tail, batch_graph, mask)
        ...
        if current_score > best_dev:
            dev_epo = idx
            if data.seg:
                print("epoch: % d, best dev f score: %.6f"%(dev_epo, current_score))
            else:
                print("Exceed previous best acc score:", best_dev)
            model_name = data.model_dir + '.' + '0' + ".model"
        ...
        if current_best > best_test:
            test_epo = idx
            best_test = current_best
        print("epoch: %d, best dev  f score: %.6f" % (dev_epo, best_dev))
        print("epoch: %d, best test f score: %.6f"% (test_epo, best_test))
        gc.collect()
