...
class Data:
    def __init__(self):
        ...
        self.HP_cuda_id = 0
        self.HP_head_layer = 1
        self.HP_tail_layer = 1
    ...
    def show_data_summary(self):
        ...
        print("     Hyper      head_layer: %s" % (self.HP_head_layer))
        print("     Hyper      tail_layer: %s" % (self.HP_tail_layer))
        print("     Hyper      star_layer: %s"%(self.HP_star_layer))
        print("     Hyper       star_head: %s"%(self.HP_star_head))
        print("     Hyper    star_dropout: %s"%(self.HP_star_dropout))
    ...
    def initial_feature_alphabets(self):
        ...
        if total_column > 4:
            for idx in range(1, total_column-3):
    ...
    def read_config(self,config_file):
        ...
        the_item = 'cuda_id'
        if the_item in config:
            self.HP_cuda_id = int(config[the_item])
        the_item = 'head_layer'
        if the_item in config:
            self.HP_head_layer = int(config[the_item])
        the_item = 'tail_layer'
        if the_item in config:
            self.HP_tail_layer = int(config[the_item])
        the_item = 'star_layer'
        if the_item in config:
            self.HP_star_layer = int(config[the_item])
        the_item = 'star_head'
        if the_item in config:
            self.HP_star_head = int(config[the_item])
        the_item = 'star_dropout'
        if the_item in config:
            self.HP_star_dropout = float(config[the_item])
        the_item = 'star_glu'
        if the_item in config:
            self.HP_star_glu = int(config[the_item])
        the_item = 'use_gcn'
        if the_item in config:
            self.use_gcn = str2bool(config[the_item])
        the_item = 'gcn_layer'
        if the_item in config:
