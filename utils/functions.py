    ...
    ### for sequence labeling data format i.e. CoNLL 2003
        for line in in_lines:
            if len(line) > 2:
                ...
                # get head/tail matrix
                boundary = list2str(label).split('-')[0]
                if boundary == 'S':
                    heads.append(1)
                    tails.append(1)
                elif boundary == 'B':
                ...
                # graph_init
                denp_heads.append(pairs[-2])
                denp_deprel.append(pairs[-1])
                ...
                if (len(words) > 0) and ((max_sent_length < 0) or (len(words) < max_sent_length)) :
                    instence_texts.append([words, features, chars, labels, heads, tails])
                    instence_Ids.append([word_Ids, feature_Ids, char_Ids, label_Ids, \
                                         heads, tails, denp_heads, denp_deprel])
                words = []
                features = []
                chars = []
                labels = []
